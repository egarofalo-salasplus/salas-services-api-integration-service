...
def etl_employees():
    ...
    logging.info(f"Datos de empleados obtenidos de SESAME - Dimensión: '{df_employees.shape}'")
    ...
    df["empresa_id"] = df["empresa_sesame_id"].apply(lambda x: get_field_id(
        field_name=x,
        serie=df_company,
        comparation_field="empresa_sesame_id",
        id_field="empresa_id")).astype(int)
    ...
    df["contrato_sesame_id"] = df_employees["contract_id"].apply(lambda x: "" if pd.isna(x) else x)
    df["nid"] = df_employees["nid"].apply(lambda x: "" if pd.isna(x) else x)
    df["ssn"] = df_employees["ssn"].apply(lambda x: "" if pd.isna(x) else x)
    df["costo_hora"] = df_employees["price_per_hour"].apply(lambda x: 0.0 if pd.isna(x) else x)
    df["fecha_nacimiento"] = pd.to_datetime(df_employees["date_of_birth"], utc=True, dayfirst=False).dt.date
    df["area"] = df_employees["cf_area"].apply(lambda x: "" if pd.isna(x) else x)
    df["costo_hora_empresa"] = df_employees["cf_precio_hora_empresa"].apply(lambda x: None if pd.isna(x) else (x if type(x) == float else float(x.replace(",", "."))))
    df["fecha_alta"] = pd.to_datetime(df_employees["cf_fecha_de_alta"], utc=True, dayfirst=True).dt.date
    df["nucleo_negocio"] = df_employees["cf_nucleo_de_negocio"].apply(lambda x: "" if pd.isna(x) else x)
    ...
    df["num_hijos"] = df_employees["children"].apply(lambda x: 0 if pd.isna(x) else int(x))
    df["porcentaje_discapacidad"] = df_employees["disability"].apply(lambda x: 0.0 if pd.isna(x) else float(x))
    df["direccion"] = df_employees["address"].apply(lambda x: "" if pd.isna(x) else x)
    df["codigo_postal"] = df_employees["postal_code"].apply(lambda x: "" if pd.isna(x) else x)
    ...
    df["nacionalidad"] = df_employees["nationality"].apply(lambda x: "" if pd.isna(x) else x)
    df["estatus_marital"] = df_employees["marital_status"].apply(lambda x: "" if pd.isna(x) else x)
    df["nivel_estudio"] = df_employees["study_level"].apply(lambda x: "" if pd.isna(x) else x)
    df["codigo_categoria_profesional"] = df_employees["professional_category_code"].apply(lambda x: "" if pd.isna(x) else x)
    df["categoria_profesional"] = df_employees["professional_category_description"].apply(lambda x: "" if pd.isna(x) else x)
    df["bic"] = df_employees["bic"].apply(lambda x: "" if pd.isna(x) else x)
    df["cargo_trabajo_sesame_id"] = df_employees["job_charge_id"].apply(lambda x: "" if pd.isna(x) else x)
    df["cargo_trabajo"] = df_employees["job_charge_name"].apply(lambda x: "" if pd.isna(x) else x)
    df["tipo_nid"] = df_employees["identity_number_type"].apply(lambda x: "" if pd.isna(x) else x)
    df["numero_cuenta"] = df_employees["account_number"].apply(lambda x: "" if pd.isna(x) else x)
    df["rango_salarial"] = df_employees["salary_range"].apply(lambda x: "" if pd.isna(x) else x)

    # Verificar orden de columnas
    df = df[["empleado_sesame_id", "nombre", "apellidos",
             "email", "telefono", "telefono_corto", "empresa_sesame_id",
             "sexo", "contrato_sesame_id", "nid", "ssn", "costo_hora",
             "fecha_nacimiento", "area", "costo_hora_empresa", "fecha_alta",
             "nucleo_negocio", "estudios", "estado", "num_hijos",
             "porcentaje_discapacidad", "direccion", "codigo_postal", "ciudad",
             "provincia", "pais", "nacionalidad", "estatus_marital",
             "nivel_estudio", "codigo_categoria_profesional",
             "categoria_profesional", "bic", "cargo_trabajo_sesame_id",
             "cargo_trabajo", "empresa_id", "tipo_nid", "numero_cuenta",
             "rango_salarial"]]
    ...
    with engine.connect() as connection:
        ...
        if not inspect(engine).has_table(table_name, schema=schema):
            ...
            df_table_new = table_df[~table_df.set_index(["empleado_sesame_id"]).index.isin(
            ...
            if not df_table_new.empty:
                ...
                logging.info(f"La tabla {table_name} ya está actualizada. No se agregaron registros nuevos.")
            ...
            if not df_table_existing_to_update.empty:
                for _, row in df_table_existing_to_update.iterrows():
                    ...
                    current_row = df_table_existing[df_table_existing["empleado_sesame_id"] == row["empleado_sesame_id"]].iloc[0]
                    
                    # Crear un diccionario con los valores a actualizar
                    params = {col: (None if pd.isna(row[col]) else row[col]) for col in table_df.columns}

                    # Verificar si los valores han cambiado
                    has_changed = any(
                        current_row[col] != row[col]
                        for col in table_df.columns
                        if col in current_row and col != "empleado_sesame_id"  # Excluir columnas de claves primarias
                    )
                    ...
                    if has_changed:
                        ...
                        logging.info(f"Revisando actualizaciones en empleado_sesame_id: {row['empleado_sesame_id']}")                        
                        connection.execute(text(update_query), params)
                    else:
                        logging.info(f"No hay cambios para empleado_sesame_id: {row['empleado_sesame_id']}")

                logging.info("Registros existentes actualizados con éxito.")
            else:
                logging.info(f"No se encontraron registros existentes para actualizar en la tabla {table_name}.")
